        ...
        $rootScope.$on('$routeChangeSuccess', function(event, args) {
            ...
            if(route && route.segment) {
                ...
                for(var i=0; i < segmentNameChain.length; i++) {
                    ...
                    var isParamsChanged = false;

                    if (newSegment.children == undefined && prevSegmentParams != params) {
                        prevSegmentParams = params;
                        isParamsChanged = true;
                    }

                    if(resolvingSemaphoreChain[i] != newSegment.name || updates.length > 0 || isDependenciesChanged(newSegment) || isParamsChanged) {

                        if($routeSegment.chain[i] && $routeSegment.chain[i].name == newSegment.name &&
                            updates.length == 0 && !isDependenciesChanged(newSegment) && isParamsChanged)
